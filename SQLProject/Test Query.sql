...
--begin try 
		Exec Instructor.CreateExam 'Exam','2024-01-15','23:00',170,100,1,1,1
--end try
--begin catch
--print 'Adsf'
--end catch
...
-- @InstructorID, @CourseID, @RandomSelection, @ExamID, @NumberOfRandomQuestion, @QuestionDegrees
		Exec Instructor.AddQuestions_Proc 1, 1, 'Random', 8 , 3 , @QuestionDeg;
-----------------------

--Add Student to exam 
--istructor_id,student_id,exam_id
	Exec Instructor.AddStudentsToExam   1, '2',8;
...
VALUES ('T13', 'True'),('X30', 'Double rectangle');
-- @StudentID,@ExamID
	EXEC Student.StoreStudentAnswers  2, 8, @StudentAnswers;
------------------------------------------
--Update Result of student

--student id,exam id 
exec UpdateResults 2,1
